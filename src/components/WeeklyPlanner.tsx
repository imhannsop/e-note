...
function Check() {
  return (
    <svg
      viewBox="0 0 24 24"
      className="check-pop size-[70%] text-[var(--accent)]"
      fill="none"
      stroke="currentColor"
      strokeWidth="3"
      strokeLinecap="round"
...
export default function WeeklyPlanner({
  ...
  const [data, setData, sync] = useDoc<Data>(
    "planner",
    `ink:planner:${profile.id}`,
    { tasks: [], checks: {} },
  );
  ...
  function rate(w: string) {
    ...
    const done = ts.reduce(
      (n, t) => n + ds.filter((d) => data.checks[`${t.id}:${d}`]).length,
      0,
    );
    const fullTasks = ds.length
      ? ts.filter((t) => ds.every((d) => data.checks[`${t.id}:${d}`])).length
      : 0;
    return {
      done,
      total: ts.length * ds.length,
      fullTasks,
      taskCount: ts.length,
    };
  }
  ...
  const pointsFrom = (end: string, n: number): Point[] =>
    Array.from({ length: n }, (_, i) => {
      ...
      return {
        day,
        done: ts.filter((t) => data.checks[`${t.id}:${day}`]).length,
        total: ts.length,
      };
    });
  ...
  function addTask() {
    ...
    setData((d) => ({
      ...
      tasks: [
        ...d.tasks,
        { id: crypto.randomUUID(), name, repeat, from: week },
      ],
    }));
    setDraft("");
  }
      ...
      tasks: d.tasks.map((x) =>
        x.id === t.id
          ? {
              ...x,
              repeat: !x.repeat,
              until: x.repeat ? nextWeek(week) : undefined,
            }
          : x,
      ),
    }));
      ...
      >
        {view === "month" ? (
          ...
          <div
            ...
            style={{
              gridTemplateColumns:
                "minmax(6.5rem,34%) repeat(7, minmax(0,1fr))",
            }}
        ...
        <div className="flex flex-wrap items-baseline justify-between gap-x-4 gap-y-1 px-1 pt-3 pb-4 text-xs text-[var(--gray)]">
          <span>
            ...
            {thisRate.taskCount} tasks done every day so far this week
          </span>
      ...
      >
        <button
          ...
          className={`flex h-10 shrink-0 items-center gap-1.5 rounded-full px-3 text-xs font-medium transition-all duration-200 active:scale-[0.97] ${
            repeat
              ? "ink-solid"
              : "border border-foreground/30 text-[var(--gray)]"
          }`}
        >
    ...
    <>
      ...
      >
        ...
        {task.repeat && (
          <Icon
            d={ICONS.repeat}
            className="size-3 shrink-0 text-[var(--gray)]"
          />
        )}
      </button>
      ...
      {open && (
        <div
          className="enter flex items-center gap-2 bg-[var(--paper)] px-3 py-2"
          style={{ gridColumn: "1 / -1" }}
        >
          <button
            type="button"
...
function MonthView({
  ...
  const dayTasks = (d: string) =>
    tasks.filter((t) => activeIn(t, weekOfKey(d)));
    ...
    <div className="flex flex-col gap-4">
      <div className="grid grid-cols-7 gap-px overflow-hidden rounded-2xl border border-[var(--ink)] bg-[var(--ink)]">
        {DAY_LETTERS.map((l, i) => (
          <div
            key={i}
            className="border-b-2 border-[var(--ink)] bg-[var(--paper)] py-2 text-center text-[11px] font-bold"
          >
            {l}
          </div>
        ))}
        {Array.from({ length: lead }, (_, i) => (
          <div
            key={`lead-${i}`}
            className="bg-[color-mix(in_srgb,var(--ink)_4%,var(--paper))]"
          />
        ))}
        {days.map((d) => {
          const ts = dayTasks(d);
          ...
          return (
            <button
              ...
              className={`relative flex aspect-square min-w-0 flex-col items-center justify-between p-1 transition-colors duration-150 ${
                isSel
                  ? "bg-[color-mix(in_srgb,var(--ink)_10%,var(--paper))]"
                  : "bg-[var(--paper)]"
              }`}
            >
              <span
              ...
              {all ? (
                ...
                <span className="mb-1 h-1 w-3/4 overflow-hidden rounded-full bg-[color-mix(in_srgb,var(--ink)_12%,var(--paper))]">
                  <span
                    ...
                    style={{ width: `${(done / ts.length) * 100}%` }}
                  />
                </span>
              ) : (
                <span className="h-2" />
              ...
              {isSel && (
                <span className="pointer-events-none absolute inset-0 border-2 border-[var(--ink)]" />
              )}
            </button>
          );
        })}
      ...
      <div className="flex flex-col gap-2">
        <div className="flex items-baseline justify-between gap-2">
          ...
          <span className="text-xs text-[var(--gray)] tabular-nums">
            {selTasks.filter((t) => checks[`${t.id}:${selected}`]).length}/
            {selTasks.length} done
          </span>
        </div>
        {selTasks.length === 0 ? (
          ...
          <ul className="flex flex-col overflow-hidden rounded-2xl border border-[var(--ink)]">
            {selTasks.map((t, i) => {
              ...
              return (
                <li
                  key={t.id}
                  className={i ? "border-t border-foreground/15" : ""}
                >
                  <button
                    type="button"
                    aria-pressed={done}
                  ...
                  >
                    <span
                      className={`grid size-6 shrink-0 place-items-center rounded-md border transition-colors duration-200 ${
                          ...
                          ? "border-[var(--gray)] bg-[color-mix(in_srgb,var(--gray)_18%,var(--paper))]"
                          : "border-[var(--ink)]"
                      }`}
                    >
                      {done && <Check />}
                    ...
                    <span
                      className={`min-w-0 flex-1 text-sm break-words ${done ? "text-[var(--gray)]" : ""}`}
                    >
                      {t.name}
                    </span>
                    {t.repeat && (
